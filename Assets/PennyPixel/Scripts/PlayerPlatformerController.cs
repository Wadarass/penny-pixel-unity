...
public class PlayerPlatformerController : PhysicsObject, IPlayerActions
{
    ...
    {
        var damageEvents = other.GetComponentInParent<IDamageEvents>();
        if (damageEvents != null)
        {
            ApplyHealthDamage(damageEvents.OnPlayerDamage());
        }
    ...
    private void ApplyHealthDamage(int healthMinus)
    {
        Energy = Mathf.Clamp(Energy -= healthMinus, 0, 100);
        eventSystemMessages.OnPlayerHurt(Energy);
    }
